...
import { getTotalFeedbackCount } from '../../../utils/supabaseApi';

const HeroSection = () => {
  const [globalFeedbackCount, setGlobalFeedbackCount] = useState(0);
  const [isLoadingCount, setIsLoadingCount] = useState(true);

  // Fetch real feedback count and update periodically
  useEffect(() => {
    const fetchRealCount = async () => {
      try {
        const realCount = await getTotalFeedbackCount();
        setGlobalFeedbackCount(realCount);
        setIsLoadingCount(false);
      } catch (error) {
        console.error('Failed to fetch real feedback count:', error);
        // Fallback to a reasonable number if fetch fails
        setGlobalFeedbackCount(42);
        setIsLoadingCount(false);
      }
    };
    ...
    const interval = setInterval(fetchRealCount, 30000);
          ...
          <div className="space-y-4">
            <div className="inline-flex items-center space-x-2 glass-card px-4 py-2 rounded-full">
              ...
              <span className="text-sm font-mono text-muted-foreground">
                {isLoadingCount ? (
                  <>Live: <Icon name="Loader2" size={12} className="inline animate-spin ml-1" /> feedback submissions</>
                ) : (
